...
```env
# Fitbit OAuth  — https://dev.fitbit.com/apps/new
...
# Polar OAuth   — https://www.polar.com/accesslink-api/
...
When registering your app on each developer portal, set the redirect URI to:
```
https://your-domain.com/api/trackers/oauth/fitbit/callback
https://your-domain.com/api/trackers/oauth/whoop/callback
https://your-domain.com/api/trackers/oauth/polar/callback
```

### How data flows

| Tracker | Method | What happens |
|---------|--------|-------------|
| HART Ring | Web Bluetooth | App connects directly via BLE, reads HR/SpO₂/HRV/temp in real time |
| Samsung Galaxy Ring/Watch | Health Connect | User's Samsung Health app → Android Health Connect → your app reads it |
| Garmin | Health Connect | Garmin Connect app → Android Health Connect → your app reads it |
| Fitbit | OAuth API | Server fetches from api.fitbit.com using stored OAuth token |
| WHOOP | OAuth API | Server fetches from api.prod.whoop.com |
| Polar | OAuth API | Server fetches from polaraccesslink.com |
| Apple Watch | N/A | Requires native iOS app; workaround: export to Google Fit → Health Connect |

### New API endpoints

```
GET  /api/trackers/status                    — which providers are connected
GET  /api/trackers/data?days=7               — merged metrics for last N days
GET  /api/trackers/oauth/:provider           — start OAuth flow (redirect)
GET  /api/trackers/oauth/:provider/callback  — OAuth callback (handled by server)
POST /api/trackers/sync/:provider            — manually trigger server-side sync
POST /api/trackers/healthconnect/sync        — push Health Connect data from client
POST /api/trackers/ble/sync                  — push BLE ring data from client
DELETE /api/trackers/:provider               — disconnect & remove tokens
```
