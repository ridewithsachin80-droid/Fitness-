...
async function run() {
  ...
  await test('POST /auth/verify-otp — wrong OTP returns 400', async () => {
    const r = await request('POST', '/api/auth/verify-otp', { phone: process.env.SMOKE_PATIENT_PHONE || '0000000000', otp: '000000' });
  ...
  await test('POST /auth/login — valid monitor login', async () => {
    const r = await request('POST', '/api/auth/login', {
      email:    process.env.ADMIN_EMAIL    || process.env.MONITOR_EMAIL,
      password: process.env.ADMIN_PASSWORD || process.env.MONITOR_PASSWORD,
    });
