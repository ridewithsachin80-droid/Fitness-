...
const STATE_STYLE = {
  ...
  normal: 'text-gold-light bg-ok/10 border-ok/30',
};
...
export default function LabResults({ memberId = null, memberName = '' }) {
  ...
  if (loading) return <p className="text-xs text-lo py-4 text-center">Loading…</p>;
    ...
    <div>
      ...
      {notice && (
        <div className="bg-amber-400/[0.08] border border-amber-400/30 rounded-xl px-3 py-2.5 mb-3">
          <p className="text-caption text-amber-200 leading-relaxed">{notice}</p>
        </div>
      )}
      ...
      {flags.length > 0 && (
        <div className="mb-3">
          <p className="text-eyebrow font-bold tracking-wider text-lo mb-1.5">
            Outside reference range
          </p>
          <div className="space-y-1.5">
            {flags.map((f, i) => (
              <div key={i} className={`flex items-center justify-between rounded-xl px-3 py-2 border ${STATE_STYLE[f.state]}`}>
                <span className="text-caption font-bold">{f.test_name}</span>
                <span className="text-caption">
                  {f.value}{f.unit ? ` ${f.unit}` : ''}
                  {f.ref && <span className="text-lo"> · ref {f.ref}</span>}
                </span>
              </div>
            ))}
          </div>
      ...
      {rawLabs.length > 0 && (
        <div className="flex gap-2 mb-3">
          <button
            ...
            style={{ minHeight: 38 }}
            className="flex-1 rounded-xl text-caption font-bold text-gold
          ...
          <button
            ...
            style={{ minHeight: 38 }}
            className="flex-1 rounded-xl text-caption font-bold text-mid
      ...
      {exportMsg && <p className="text-eyebrow text-lo mb-3">{exportMsg}</p>}
        ...
        <div className="mb-3">
          {!insight && (
            <button onClick={generateInsight} disabled={thinking} style={{ minHeight: 42 }}
              className="w-full rounded-xl text-xs font-bold text-gold
                bg-gold/[0.08] border border-gold/[0.28]
          ...
          {insight && !insight.error && insight.urgent?.length > 0 && (
            <div className="bg-red-500/[0.08] border border-red-500/35 rounded-xl px-3.5 py-3">
              <p className="text-caption font-bold tracking-wider text-red-300 mb-1.5">
                Needs medical review first
              </p>
              <p className="text-note text-white leading-relaxed mb-2">{insight.summary}</p>
              <div className="space-y-1 mb-2">
                {insight.urgent.map((u, i) => (
                  <div key={i} className="flex justify-between text-caption">
                    <span className="text-white">{u.test_name}</span>
              ...
              <p className="text-eyebrow text-mid leading-relaxed">{insight.note}</p>
            </div>
          )}

          {insight?.generated && (
            <div className="bg-charcoal border border-white/[0.08] rounded-xl p-3.5">
              <p className="text-note text-white leading-relaxed mb-3">{insight.summary}</p>

              {(insight.markers || []).map((m, i) => (
                <div key={i} className="mb-3 pb-3 border-b border-white/[0.06] last:border-0 last:pb-0">
                  <p className="text-caption font-bold text-gold mb-1">{m.test_name}</p>
                  <p className="text-caption text-mid leading-relaxed mb-1">{m.what_it_is}</p>
                  <p className="text-caption text-white leading-relaxed">{m.diet_change}</p>
                  {m.timeframe && (
                    <p className="text-eyebrow text-lo mt-1">{m.timeframe}</p>
                  )}
                </div>
              ))}
              ...
              {insight.macro_targets && (
                <div className="bg-surface border border-gold/25 rounded-xl p-3 mb-3">
                  <div className="flex items-baseline justify-between mb-2">
                    <p className="text-eyebrow font-bold tracking-wider text-gold">
                      Macro target
                    </p>
                  ...
                  <div className="flex h-2 rounded-full overflow-hidden mb-2">
                    <div style={{ width: `${insight.macro_targets.protein_pct}%` }} className="bg-gold" />
                    <div style={{ width: `${insight.macro_targets.carbs_pct}%` }} className="bg-gold-dark" />
                    <div style={{ width: `${insight.macro_targets.fat_pct}%` }} className="bg-gold-light" />
                  </div>

                  <div className="grid grid-cols-3 gap-2 text-center">
                    {[
                      ['Protein', insight.macro_targets.protein_g, insight.macro_targets.protein_pct, 'text-gold'],
                      ['Carbs',   insight.macro_targets.carbs_g,   insight.macro_targets.carbs_pct,   'text-gold-deep'],
                      ['Fat',     insight.macro_targets.fat_g,     insight.macro_targets.fat_pct,     'text-gold-light'],
                    ].map(([label, g, pctv, cls]) => (
                      <div key={label}>
                        <p className={`text-sm font-extrabold ${cls}`}>{pctv}%</p>
                        <p className="text-caption text-white">{g}g</p>
                        <p className="text-tiny tracking-wider text-lo">{label}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  {(insight.macro_targets.reasons || []).length > 0 && (
                    <ul className="mt-2 space-y-0.5">
                      {insight.macro_targets.reasons.map((r, i) => (
                        <li key={i} className="text-eyebrow text-mid leading-relaxed">· {r}</li>
                      ))}
                    </ul>
                  )}
                </div>
                ...
                <>
                  ...
                  <div className="space-y-1.5 mb-3">
                    {insight.meal_ideas.map((m, i) => (
                      <div key={i} className="bg-surface border border-white/[0.06] rounded-lg px-3 py-2">
                        <p className="text-caption font-bold text-white">{m.meal}</p>
                        <p className="text-caption text-mid leading-relaxed">{m.idea}</p>
                        {m.why && <p className="text-eyebrow text-gold mt-0.5">{m.why}</p>}
                      </div>
                    ))}
                  </div>
                </>
              ...
              {(insight.raise_with_doctor || []).length > 0 && (
                <p className="text-caption text-amber-300 leading-relaxed mb-2">
                  Raise with their doctor: {insight.raise_with_doctor.join(', ')}
                </p>
              )}

              <p className="text-eyebrow text-lo leading-relaxed">{insight.caveat}</p>
          ...
          {insight && !insight.error && !insight.generated && !insight.urgent?.length && (
            <p className="text-xs text-mid leading-relaxed">{insight.summary}</p>
          )}
        </div>
      )}
          ...
          {comparisons.map((c, i) => (
            <div key={i} className="bg-charcoal border border-hair rounded-xl p-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-eyebrow font-bold tracking-wider ${
                  c.direction === 'improved' ? 'text-gold-light'
                  : c.direction === 'worsened' ? 'text-amber-300' : 'text-mid'}`}>
                  {c.direction}
                </span>
              </div>
              <p className="text-caption text-mid">
                {c.from} → <span className="text-white font-semibold">{c.to}</span>
                {c.unit ? ` ${c.unit}` : ''}
                {c.from_state && c.to_state && c.from_state !== c.to_state && (
                  <span className="text-gold"> · {c.from_state} → {c.to_state}</span>
                )}
                <span className="text-lo"> · {c.interval_days} days</span>
              </p>

              {/* What was happening in that window */}
              <div className="mt-2 pt-2 border-t border-white/[0.06]">
                <p className="text-tiny font-bold tracking-wider text-lo mb-1">
      ...
      {!isCoach && (
        adding ? (
          <div className="bg-charcoal border border-white/[0.08] rounded-xl p-3">
            <div className="flex gap-2 mb-2">
              <label className="flex-1">
                <span className="block text-tiny tracking-wider text-lo mb-1">Test date</span>
                <input type="date" value={testDate} max={new Date().toISOString().slice(0, 10)}
                  ...
                  className="w-full bg-surface border border-white/[0.12] rounded-lg px-2 text-xs text-white" />
              </label>
              <label className="flex-1">
                <span className="block text-tiny tracking-wider text-lo mb-1">Lab (optional)</span>
                <input value={labName} onChange={e => setLabName(e.target.value)} placeholder="e.g. Metropolis"
                  style={{ minHeight: 38 }}
                  className="w-full bg-surface border border-white/[0.12] rounded-lg px-2 text-xs text-white placeholder-lo" />
              </label>
            </div>

            {rows.map((r, i) => (
              <div key={i} className="bg-surface border border-hair rounded-lg p-2 mb-1.5">
                <div className="flex gap-1.5 mb-1.5">
                  <input list="common-labs" value={r.test_name}
                    ...
                    className="flex-1 min-w-0 bg-charcoal border border-white/[0.12] rounded-md px-2 text-xs text-white placeholder-lo" />
                  <input value={r.value} onChange={e => setRow(i, 'value', e.target.value)}
                    ...
                    className="bg-charcoal border border-white/[0.12] rounded-md px-2 text-xs text-center text-white placeholder-lo" />
                  {rows.length > 1 && (
                    <button onClick={() => setRows(rs => rs.filter((_, j) => j !== i))}
                      className="w-6 text-lo hover:text-red-400 text-sm">×</button>
                  )}
                </div>
                <div className="flex gap-1.5">
                  {[['unit', 'Unit'], ['ref_min', 'Ref min'], ['ref_max', 'Ref max']].map(([f, ph]) => (
                    <input key={f} value={r[f]} onChange={e => setRow(i, f, e.target.value)}
                      ...
                      className="flex-1 min-w-0 bg-charcoal border border-white/[0.10] rounded-md px-2 text-eyebrow text-center text-mid placeholder-lo" />
                  ))}
                </div>
              </div>
            ))}
            ...
            <button onClick={() => setRows(rs => [...rs, { test_name: '', value: '', unit: '', ref_min: '', ref_max: '' }])}
              className="text-caption font-bold text-gold mb-2">+ Another result</button>
      ...
      {data?.caveat && (
        <p className="text-eyebrow text-lo mt-3 leading-relaxed">{data.caveat}</p>
      )}
    </div>
  );
}
