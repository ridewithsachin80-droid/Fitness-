    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end justify-center p-2">
      <div className="bg-charcoal rounded-3xl border border-white/[0.08] w-full max-w-md max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          ...
          {templates.length > 0 && (
            <div>
              <p className="text-eyebrow text-dim font-semibold tracking-wide mb-1.5">Use a template</p>
              <div className="flex gap-1.5 flex-wrap">
                {templates.map(t => (
                  <button key={t.id} onClick={() => handleUseTemplate(t.id)} disabled={saving}
                    className="px-3 py-1.5 text-xs font-semibold rounded-full bg-white/[0.06] border border-white/[0.1] text-soft hover:bg-white/[0.1] transition-colors disabled:opacity-50">
                    {t.name}
                  </button>
                ))}
              </div>
              <p className="text-eyebrow text-dim mt-2">— or build a custom program below —</p>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center gap-1.5 flex-wrap mb-2">
              ...
              <button onClick={addDay} className="px-3 py-1.5 text-xs font-semibold rounded-full bg-white/[0.04] border border-dashed border-white/[0.15] text-mid hover:text-white">
                + Day
              </button>
            </div>

            {/* Active day editor */}
            <div className="bg-white/[0.02] border border-hair rounded-xl p-3">
              <div className="flex items-center gap-2 mb-3">
                <input value={days[activeDayIdx].day_label} onChange={e => updateDayLabel(e.target.value)}
                  ...
                  className="flex-1 px-2.5 py-1.5 bg-surface border border-white/[0.1] rounded-lg text-xs text-soft focus:outline-none focus:ring-2 focus:ring-gold/30" />
                {days.length > 1 && (
                  <button onClick={() => removeDay(activeDayIdx)} className="text-dim hover:text-red-400 text-xs">Remove day</button>
                )}
              </div>
                  ...
                  <div className="absolute left-0 right-0 top-full mt-1 bg-surface border border-white/[0.1] rounded-lg shadow-lg z-10 max-h-40 overflow-y-auto">
                    {results.map(r => (
                      <button key={r.id} onClick={() => addExerciseToDay(r)}
                        className="w-full text-left px-2.5 py-2 text-xs text-soft hover:bg-white/[0.05]">
                        {r.name}
                      </button>
                    ))}
                  </div>
              ...
              {days[activeDayIdx].exercises.length === 0 ? (
                <p className="text-xs text-dim text-center py-3">No exercises yet — search above to add some.</p>
              ) : (
                <div className="space-y-2">
                  {days[activeDayIdx].exercises.map(ex => (
                    <div key={ex.exercise_id} className="flex items-center gap-2 bg-white/[0.02] rounded-lg p-2">
                      <span className="flex-1 text-xs text-soft">{ex.exercise_name}</span>
                      <input type="number" min="1" value={ex.target_sets}
                        onChange={e => updateExerciseTarget(ex.exercise_id, 'target_sets', e.target.value)}
                        className="w-10 px-1 py-1 bg-surface border border-white/[0.1] rounded text-xs text-center text-bright" />
                      ...
                      <input type="number" min="1" value={ex.target_reps_min}
                        ...
                        className="w-10 px-1 py-1 bg-surface border border-white/[0.1] rounded text-xs text-center text-bright" />
                      <button onClick={() => removeExerciseFromDay(ex.exercise_id)} className="text-dim hover:text-red-400 text-sm px-1">×</button>
                    </div>
                  ))}
                </div>
              )}
