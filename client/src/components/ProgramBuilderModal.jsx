            ...
            <div className="bg-white/[0.02] border border-white/[0.07] rounded-xl p-3">
              <div className="flex items-center gap-2 mb-3">
                <input value={days[activeDayIdx].day_label} onChange={e => updateDayLabel(e.target.value)}
                  ...
                  className="flex-1 px-2.5 py-1.5 bg-[#1A1C20] border border-white/[0.1] rounded-lg text-xs text-[#d8d8de] focus:outline-none focus:ring-2 focus:ring-[rgba(212,175,55,0.30)]" />
                {days.length > 1 && (
              ...
              <div className="relative mb-2">
                <input value={search} onChange={e => runSearch(e.target.value)}
                  ...
                  className="w-full px-2.5 py-2 bg-[#1A1C20] border border-white/[0.1] rounded-lg text-xs text-[#d8d8de] placeholder-[#5a5a68] focus:outline-none focus:ring-2 focus:ring-[rgba(212,175,55,0.30)]" />
                {results.length > 0 && (
                  <div className="absolute left-0 right-0 top-full mt-1 bg-[#1A1C20] border border-white/[0.1] rounded-lg shadow-lg z-10 max-h-40 overflow-y-auto">
                    {results.map(r => (
                  ...
                  {days[activeDayIdx].exercises.map(ex => (
                    <div key={ex.exercise_id} className="flex items-center gap-2 bg-white/[0.02] rounded-lg p-2">
                      ...
                      <input type="number" min="1" value={ex.target_sets}
                        ...
                        className="w-10 px-1 py-1 bg-[#1A1C20] border border-white/[0.1] rounded text-xs text-center text-[#ededf0]" />
